...
const config: ForgeConfig = {
  packagerConfig: {
    ...
    extraResource: [
      // Athena binary - only include when building for production
      // path.join(__dirname, '../dist/darwin/athena'),
      // ...(fs.existsSync(path.join(__dirname, '../dist/darwin-amd64/lib/athena')) 
      //   ? fs.readdirSync(path.join(__dirname, '../dist/darwin-amd64/lib/athena')).map(f => path.join(__dirname, '../dist/darwin-amd64/lib/athena', f))
      //   : []),

...
struct TribunalDecisionWindow: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            Text("Case: \(`case`.caseID)").font(.headline)
            Text(`case`.summary).font(.body)
            Text("AI Recommendation: \(`case`.aiRecommendation.rawValue.uppercased()) (\(Int(`case`.confidence * 100))%)")

...
final class GovernanceAPIClient {
    ...
    func setMode(_ mode: GovernanceMode) async throws {
        ...
        let body = ["mode": mode.rawValue, "timestamp": ISO8601DateFormatter().string(from: Date())]
    ...
    func fetchKPIs() async -> GovernanceKPIs {
        ...
        async let hardFails5m = queryPrometheus("sum(increase(governance_verdicts_total{verdict_type=\"hard_fail\"}[5m]))")

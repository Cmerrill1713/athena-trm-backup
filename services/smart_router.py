...
class SmartRouter:
    ...
    def route_query(self, query: str, max_tokens: int = 500) -> Dict[str, any]:
        """Main routing function - analyze query and select optimal model + RAG"""
        start_time = time.time()
        
        # Analyze the query
        query_analysis = self.analyze_query(query)
        
        # Check if query needs RAG (factual, knowledge-based queries)
        needs_rag = self._should_use_rag(query_analysis)
    ...
    def _should_use_rag(self, query_analysis: Dict[str, any]) -> bool:
        """Determine if query should use RAG based on analysis"""
        query_type = query_analysis["query_type"]
        complexity = query_analysis["complexity_score"]
        ...
        rag_keywords = [
            ...
            "policy", "procedure", "documentation", "guide", "manual"
        ]
        ...
        # Factual questions typically need RAG
        if query_type == "explanatory" and complexity >= 2:
            return True
            
        # Policy/documentation questions
        if any(word in query_lower for word in ["policy", "document", "guide", "manual"]):
            return True

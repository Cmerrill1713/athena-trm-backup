...
struct HealthBanner: View {
    ...
    // Fallback health checks (used when ServiceRegistry isn't available at compile-time)
    private func defaultHealthChecks() -> [(String, String)] {
        return [
            ("Athena", "http://127.0.0.1:8090/health"),
    ...
    private func reconnectNow() async {
        ...
        // Rerun check using local fallback
        let checks = defaultHealthChecks()
        totalServices = checks.count
        var upCount = 0
